...
Cloud Run Hypergraph‑Vector Recommender; CPU-only FastAPI service using INT8 ONNX MiniLM, pgvector similarity, and hyperedge signals.
...
High‑Level Architecture (public summary)
...
- Dockerfile, cloudbuild.yaml, Makefile (GCP targets only).
...
- Public service address (as of 2025‑12‑07):
  ...
  - Infra service: if already deployed in your project, fetch its URL with: `gcloud run services describe infra-service --region=europe-west2 --format='value(status.url)'`.
- One‑liner health check:
  - `curl -s https://embedding-service-5pgvctvdpq-nw.a.run.app/healthz`
- Example: embed text
  - `curl -s -X POST https://embedding-service-5pgvctvdpq-nw.a.run.app/embed/text -H 'content-type: application/json' -d '{"text":"A sci-fi thriller with mind-bending twists."}'`

API Summary (JSON FastAPIs)
...
GCP‑Only Deployment
- Assumptions: run from Google Cloud Shell with project set. No local `gcloud auth` instructions are required or used.
- Keep existing names/links from the current deployment:
  - Project: `agentics-foundation25lon-1809`; Region: `europe-west2`.
  ...
  - Service accounts: `embedding-service@agentics-foundation25lon-1809.iam.gserviceaccount.com` and `embedding-jobs@agentics-foundation25lon-1809.iam.gserviceaccount.com`.
...
Agent Playbook — Deploy and Test Services
- Pre‑set context (Cloud Shell):
  - `export PROJECT_ID=agentics-foundation25lon-1809; export REGION=europe-west2`
...
- Deploy embeddings‑service and fetch URL:
  ...
  - Embed example: `curl -s -X POST "$SERVICE_URL/embed/text" -H 'content-type: application/json' -d '{"text":"A cozy mystery with dry humor."}'`
- Deploy infra‑service and fetch URL:
  ...
  - Graph example: `curl -s -X POST "$INFRA_URL/graph/recommend" -H 'content-type: application/json' -d '{"query":"mind‑bending sci‑fi thriller","top_k":10,"seed_top_k":50,"hops":2}'`
- Notes for agents:
...
- Build → Deploy `embedding-service` → Smoke test:
  - `PROJECT_ID=$PROJECT_ID REGION=$REGION make gcp-build && make gcp-deploy && gcloud run services describe embedding-service --region=$REGION --format='value(status.url)'`
  - `curl -s "$SERVICE_URL/healthz"`
- Deploy `infra-service` and call graph endpoint:
  - `PROJECT_ID=$PROJECT_ID REGION=$REGION make gcp-deploy-infra && INFRA_URL=$(gcloud run services describe infra-service --region=$REGION --format='value(status.url)')`
  - `curl -s -X POST "$INFRA_URL/graph/recommend" -H 'content-type: application/json' -d '{"query":"mind‑bending sci‑fi thriller","top_k":10,"seed_top_k":50,"hops":2}'`

Data Sourcing (Kaggle/TMDB)
...
- Recommended GCS layout (already used in this project):
...
Model Sourcing (MiniLM) and Formats
...
- Hyperedge build is two‑phase and bounded to keep memory stable:
  - Generate per‑part co‑watch edges, persist to `hyperedges_parts/`.
  - Aggregate top‑K per source, add genre edges, emit `hyperedges.parquet`, then optionally insert to DB in chunks.
...
Configuration Options (kept from original)
- Environment variables:
  ...
  - `MODEL_GCS_URI` or `GCS_MODELS_BUCKET` to pull ONNX artifacts from GCS; else local `BASE_MODEL_DIR` cache is used for fallback ST embedding.
  - `EMBEDDING_BACKEND=auto|onnx|st|hash` (default `auto`).
  - `USE_GRAPH_SCORER=true|false` and `GRAPH_SCORE_WEIGHT`.
  - `USE_RERANKER=true|false`.
  - CORS: `ALLOWED_ORIGINS` via `allowed_origins` list in settings if needed.
- Service names/links:
...
- Log every GCP action for compliance: `scripts/gcp_log.py --run "gcloud ..." --purpose "Deploy CR"` appends to `gcp-activity-log.md`.
- Cloud Run Jobs deploy helpers: `make gcp-jobs-deploy` and `make gcp-job-run-<jobname>`.

TODO (next features; includes pipeline‑phase3)
