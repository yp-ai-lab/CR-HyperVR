...
if [[ -z "$PROJECT_ID" || -z "$REGION" ]]; then
  echo "PROJECT_ID/REGION not set. Set env vars or use Cloud Shell config." >&2
  exit 1
fi
...
gcloud projects add-iam-policy-binding "$PROJECT_ID" --member="serviceAccount:$RUNTIME_SA_EMAIL" --role="roles/cloudsql.client" --quiet >/dev/null
gcloud projects add-iam-policy-binding "$PROJECT_ID" --member="serviceAccount:$RUNTIME_SA_EMAIL" --role="roles/secretmanager.secretAccessor" --quiet >/dev/null
gcloud projects add-iam-policy-binding "$PROJECT_ID" --member="serviceAccount:$RUNTIME_SA_EMAIL" --role="roles/artifactregistry.reader" --quiet >/dev/null

gcloud projects add-iam-policy-binding "$PROJECT_ID" --member="serviceAccount:$JOBS_SA_EMAIL" --role="roles/cloudsql.client" --quiet >/dev/null
gcloud projects add-iam-policy-binding "$PROJECT_ID" --member="serviceAccount:$JOBS_SA_EMAIL" --role="roles/secretmanager.secretAccessor" --quiet >/dev/null
gcloud projects add-iam-policy-binding "$PROJECT_ID" --member="serviceAccount:$JOBS_SA_EMAIL" --role="roles/storage.admin" --quiet >/dev/null
...
if gcloud sql databases describe "$DB_NAME" --instance "$SQL_INSTANCE" >/dev/null 2>&1; then :; else
